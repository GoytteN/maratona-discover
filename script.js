...
const Transaction = {
  ...
  incomes() {
    ...
    Transaction.all.forEach((transaction) => {
      transaction.amount > 0 ? (income += transaction.amount) : 0;
    });
    return income;
  },
  ...
  expenses() {
    let expense = 0;
    Transaction.all.forEach((transaction) => {
      transaction.amount < 0 ? (expense += transaction.amount) : 0;
    });
    return expense;
  },
...
const DOM = {
  ...
  verifyValue() {
    let value = Transaction.total()
    console.log(value)
    let card = document.querySelector('.total')
    if (value < 0) {
      card.style.background = '#a03333'
    } else {
      card.style.background = '#49AA26'
    }
  }
};
